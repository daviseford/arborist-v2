...
export default class ArboristMain extends React.Component<{}, IArboristMainState> {
  ...
  public handleFileDone(data) {
    ...
    const dest = data.dest
      .split(path.sep)
      .slice(data.dest.split(path.sep).length - 2)
      .join(path.sep);
  ...
  public render() {
    return (
      <div className="col-xs-8 col-xs-offset-2" style={{ marginTop: '9%' }}>
        ...
        <div className="row" hidden={!this.state.filepath} style={{ marginTop: '2%' }}>
          <div className="col-xs-12 text-center">
            <div className="form-group">
              <AnalyzeButton filepath={this.state.filepath} handleSorter={this.handleSorterUpdates.bind(this)} />
        ...
        <CopyListDisplay
          copy_list={this.state.copy_list}
          isSorted={this.state.isSorted}
          filepath={this.state.filepath}
        />
...
interface IConsoleAreaProps {
  console_output: string;
}
class ConsoleArea extends React.Component<IConsoleAreaProps, {}> {
  public render() {
    return (
      <div className="row" style={{ marginTop: '2%' }}>
        <div className="col-xs-12">
          <textarea className="form-control" rows={7} value={this.props.console_output} readOnly>
            {' '}
          </textarea>
        </div>
      </div>
